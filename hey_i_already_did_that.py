...
Commander Lambda uses an automated algorithm to assign minions randomly to tasks, in order to keep her minions on their
...
starting with n. For instance, in the example above, solution(210022, 3) would return 3, since iterating on 102212 would
...
def get_new_n(n, k, b):
    ...
    y = ''.join(sorted(n))
    return str_base(int(x, base=b) - int(y, base=b), string.digits[:b]).zfill(k)

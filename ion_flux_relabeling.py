...
root, two internal nodes and four leaf nodes (like the example above), and so forth.  The lists q and p contain at
...
def solution(h, q):
    ...
    while len(nodes) > 0:
        ...
        l_child, r_child = node - 2**(height-1), node - 1
        height -= 1
        if height < 1:
            break
        parents.update({l_child: node, r_child: node})
        nodes.extendleft([(l_child, height), (r_child, height)])

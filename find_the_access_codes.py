...
contains the access codes. You need to find a way to determine which list contains the access codes once you're ready
to go in.
...
def solution(l):
    ...
    if len(l) < 3:
        return 0
    lookup = {i: [j for j in range(i + 1, length) if not (l[j] % l[i])] for i in range(length)}

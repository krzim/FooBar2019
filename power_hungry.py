...
of each panel in an array, and returns the maximum product of some non-empty subset of those numbers. So for example,
...
positive output of the multiple of their power values). The final products may be very large, so give the solution as
...
def solution(xs):
    neg = sorted(map(abs, filter(lambda x: x < 0, xs)), reverse=True)
    pos = filter(lambda x: x > 0, xs)
